                        ...
                        <div class="mb-6 p-6 bg-blue-50/50 rounded-[2rem] border border-blue-100/50 relative group/skills">
                            ...
                            <div class="flex flex-wrap gap-2">
                                @forelse($project->skills as $skill)
                                    <a href="{{ url('/?search=' . urlencode($skill->name)) }}" class="px-4 py-2 bg-white border border-blue-100 text-blue-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 hover:text-white hover:shadow-lg hover:shadow-blue-500/20 transition-all">
                                        {{ $skill->name }}
                                    </a>
                                @empty
                                    ...
                                    @if($member->memberable instanceof \App\Models\User || $member->memberable instanceof \App\Models\Circle)
                                        <div class="flex flex-wrap gap-1 mb-3">
                                            ...
                                            @foreach($uniqueAchievements as $ach)
                                                <a href="{{ $ach->skill ? url('/?search=' . urlencode($ach->skill->name)) : '#' }}" class="text-[8px] font-black text-blue-500 bg-blue-50 px-2 py-0.5 rounded-md uppercase tracking-tighter hover:bg-blue-600 hover:text-white transition-all">
                                        ...
                                        <div class="flex-grow min-w-0">
                                            ...
                                            <div class="flex flex-wrap gap-1 mb-3">
                                                ...
                                                @foreach($expertAchievements as $ach)
                                                    <a href="{{ $ach->skill ? url('/?search=' . urlencode($ach->skill->name)) : '#' }}" class="text-[8px] font-black text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md uppercase tracking-tighter hover:bg-slate-900 hover:text-white transition-all">
                                                        {{ $ach->skill->name ?? $ach->title }}
                                                    </a>
                                                @endforeach
                                            </div>
